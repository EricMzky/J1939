...
* Description:	
* Parameter:	MsgPtr->用户要出队的消息, _Can_Node        ->要出队的CAN硬件编号
				
* note 			从接受队列中读取一个信息到*MsgPtr。如果我们用的是中断，
				需要将中断失能，在获取接受队列数据时
...
* Description:	
* Parameter:	MsgPtr->存储读取消息的缓存, _Can_Node        ->读取消息的CAN硬件编号（从哪一路CAN读取数据）
...
* Description:	
* Parameter:	MsgPtr->用户要入队的消息, _Can_Node        ->要入队的CAN硬件编号（要选择的使用的CAN硬件编号）
* note 			这段程序，将*MsgPtr放入发送消息列队中,如果信息不能入队或者发送，将有一个相应的返回提示,
...
* Description:	
* Parameter:	MsgPtr->存储发送消息的缓存, _Can_Node        ->发送消息的CAN硬件编号（从哪一路CAN发送数据）
* note 			这段程序，将*MsgPtr放入发送消息列队中,如果信息不能入队或者发送，将有一个相应的返回提示,
				如果发送中断被设置（可用），当消息列队后，发送中断被使能
* Return:		RC_SUCCESS          消息入队成功 
				RC_QUEUEFULL        发送列队满，消息入队失败 
