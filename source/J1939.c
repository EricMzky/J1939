...
void SendOneMessage(J1939_MESSAGE *pstMsg);
...
UINT8 J1939_EnqueueMessage(J1939_MESSAGE *pstMsg, CAN_NODE  _Can_Node);
UINT8 J1939_DequeueMessage(J1939_MESSAGE *pstMsg, CAN_NODE  _Can_Node);
UINT8 J1939_Read_Message(J1939_MESSAGE *pstMsg, CAN_NODE  _Can_Node);
UINT8 J1939_Send_Message(J1939_MESSAGE *pstMsg, CAN_NODE  _Can_Node);
...
* Description:	
* Parameter:	pstMsg->用户要出队的消息, _Can_Node        ->要出队的CAN硬件编号
...
* Description:	
* Parameter:	pstMsg->用户要入队的消息, _Can_Node        ->要入队的CAN硬件编号（要选择的使用的CAN硬件编号）
...
* Description:	
* Parameter:	pstMsg->存储读取消息的缓存, _Can_Node        ->读取消息的CAN硬件编号（从哪一路CAN读取数据）
...
UINT8 J1939_Read_Message(J1939_MESSAGE *pstMsg, CAN_NODE _Can_Node)
{
	return J1939_DequeueMessage(pstMsg, _Can_Node);
}
...
* Description:	
* Parameter:	pstMsg->存储发送消息的缓存, _Can_Node        ->发送消息的CAN硬件编号（从哪一路CAN发送数据）
* note 			这段程序，将*pstMsg放入发送消息列队中,如果信息不能入队或者发送，将有一个相应的返回提示,
				如果发送中断被设置（可用），当消息列队后，发送中断被使能
* Return:		RC_SUCCESS          消息入队成功 
				RC_QUEUEFULL        发送列队满，消息入队失败 
...
UINT8 J1939_Send_Message( J1939_MESSAGE *pstMsg, CAN_NODE  _Can_Node)
{
	return J1939_EnqueueMessage(pstMsg,_Can_Node);
}
...
* Parameter:	
				...
				[in] _Can_Node  要入队的CAN硬件编号（要选择的使用的CAN硬件编号）
